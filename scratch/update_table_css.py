...
/* REAL POKER TABLE CONTAINER (EXPANDED MAJESTIC FELT) */
...
.poker-table-felt {
    ...
    box-shadow: inset 0 0 60px rgba(0, 0, 0, 0.95), 0 25px 75px rgba(0, 0, 0, 0.95), 0 0 40px rgba(17, 212, 131, 0.18);
    overflow: visible;
}
...
/* PLAYER SEATS — POSITIONED ON TABLE RAILS (ZERO OVERLAP) */
.player-seat {
    position: absolute;
    left: 50%;
...
if idx_start != -1 and idx_end != -1:
    new_content = content[:idx_start] + css_larger_table + '\n\n' + content[idx_end:]
    with open('styles.css', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print('SUCCESS: Updated styles.css with larger table and refined shockwave!')
else:
