        ...
        function setupPokerDealer() {
            ...
            const tl = gsap.timeline({
                scrollTrigger: {
                    trigger: section,
                    start: "top top",
                    end: "+=120%", /* Calibrated scroll runway — fast & responsive */
                    pin: true,
                    scrub: 1.0,
                    anticipatePin: 1
                }
            ...
            // PHASE 0: Reveal player seat badges
            if (seatA) tl.to(seatA, { opacity: 1, scale: 1, duration: 0.3, ease: "back.out(1.5)" }, 0.05);
            if (seatJ) tl.to(seatJ, { opacity: 1, scale: 1, duration: 0.3, ease: "back.out(1.5)" }, 0.1);

            // PHASE 1: Deal Andrés's hand (Top of Table)
            andresCards.forEach((card, index) => {
                const spot  = andresSpots[index] || { x: 0, y: -105, rZ: 0 };
                const inner = card.querySelector('.poker-card-inner');
                ...
                tl.call(() => { spawnShockwave(card, 80, 'rgba(240, 192, 48, 0.85)'); playTick(400 + index * 30, 0.06); }, null, delay + 0.78);
            });

            // PHASE 2: Deal Johan's hand (Bottom of Table)
            const johanStart = 1.0;
            johanCards.forEach((card, index) => {
                const spot  = johanSpots[index] || { x: 0, y: 105, rZ: 0 };
                const inner = card.querySelector('.poker-card-inner');
                ...
                tl.to(card, { scaleY: 0.86, scaleX: 0.96, duration: 0.08, ease: "power1.out" }, delay + 0.75);
                tl.to(card, { scaleY: 0.92, scaleX: 0.92, duration: 0.15, ease: "elastic.out(1.2, 0.6)" }, delay + 0.83);
                tl.call(() => { spawnShockwave(card, 80, 'rgba(17, 212, 131, 0.85)'); playTick(440 + index * 30, 0.06); }, null, delay + 0.78);
            });

            // PHASE 3: THE FINAL SLAM — Central VANTA Master Winning River Card
            const masterStart = johanStart + 1.0;
            if (masterCard) {
                const inner = masterCard.querySelector('.poker-card-inner');
                ...
                tl.call(() => {
                    spawnShockwave(masterCard, 200, '#f0c030');
                    triggerScreenShake();
                    playTick(880, 0.2);
...
if idx_start != -1 and idx_end != -1:
    new_content = content[:idx_start] + js_clean_code + '\n\n}\n\n' + content[idx_end:]
    with open('effects.js', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print('SUCCESS: Updated effects.js cleanly!')
else:
