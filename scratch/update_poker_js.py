...
js_poker_code = """    /* ============================================================
       AWWWARDS CYBERPUNK POKER TECH DECK — Full Poker Game Sequence
       Left Seat: ANDRÉS (Full-Stack & Cloud)
       Right Seat: JOHAN (AI Vision & 3D Graphics)
       Center: THE WINNING RIVER CARD (VANTA Master Slam)
       ============================================================ */
    (function initCyberpunkPokerDeckScrollytelling() {
        ...
        const techSpecsData = {
            ...
            postgres: {
                ...
                desc: "Base de datos relacional serverless con aislamiento de transacciones ACID estrictas e consultas JSONB híbridas.",
                projects: [
        ...
        function setupPokerDealer() {
            ...
            // ─── DOM Shockwave ────────────────────────────────────────────
            function spawnShockwave(cardEl, customSize = 120, customBorder = 'rgba(17, 212, 131, 0.9)') {
                if (!stage) return;
                const sRect = stage.getBoundingClientRect();
                const cRect = cardEl.getBoundingClientRect();
                const x = (cRect.left + cRect.width / 2) - sRect.left;
                const y = (cRect.bottom) - sRect.top - 12;

                const ripple = document.createElement('div');
                ripple.className = 'card-shockwave';
                ripple.style.cssText = `
                    left: ${x}px;
                    top: ${y}px;
                    width: ${customSize}px;
                    height: ${customSize * 0.42}px;
            ...
            // ─── Separated Player Fan Positions ───────────────────────────
            ...
            // Andrés hand fan spots (Left side of felt)
            const andresSpots = [
                { x: -500, y: 40, rZ: -16 },
                { x: -420, y: 25, rZ: -11 },
                { x: -340, y: 15, rZ: -6  },
            ...
            // Johan hand fan spots (Right side of felt)
            const johanSpots = [
                { x: 100, y: 40, rZ: -12 },
                { x: 180, y: 25, rZ: -6  },
                { x: 260, y: 15, rZ: 0   },
            ...
            // Initial hide inside dealer deck
            andresCards.forEach((card, i) => {
                const inner = card.querySelector('.poker-card-inner');
                gsap.set(card, { x: 0, y: -140, scale: 0.2, opacity: 0, rotationZ: 0, zIndex: i + 1 });
            ...
            // Master Card initial state (floating high above center)
            if (masterCard) {
                const inner = masterCard.querySelector('.poker-card-inner');
                gsap.set(masterCard, { x: 0, y: -260, scale: 2.2, opacity: 0, rotationZ: 0, zIndex: 50 });
                if (inner) gsap.set(inner, { rotateY: 0 });
            }
            ...
            if (seatA) gsap.set(seatA, { opacity: 0, y: -20 });
            if (seatJ) gsap.set(seatJ, { opacity: 0, y: -20 });
            ...
            // PHASE 0: Header text fade out
            const pokerHeader = section.querySelector('.poker-header');
            const dealerDeck  = section.querySelector('.poker-dealer-deck');
            if (pokerHeader) {
                gsap.set(pokerHeader, { opacity: 1, y: 0, scale: 1, filter: "blur(0px)" });
                tl.to(pokerHeader, { opacity: 0, y: -40, scale: 0.95, filter: "blur(8px)", duration: 0.4, ease: "power2.inOut" }, 0);
            }
            if (dealerDeck) {
                gsap.set(dealerDeck, { opacity: 1, scale: 1 });
                tl.to(dealerDeck, { opacity: 0.2, scale: 0.8, duration: 0.4, ease: "power2.inOut" }, 0);
            }

            // Reveal Player Seats
            if (seatA) tl.to(seatA, { opacity: 1, y: 0, duration: 0.4, ease: "power2.out" }, 0.2);
            if (seatJ) tl.to(seatJ, { opacity: 1, y: 0, duration: 0.4, ease: "power2.out" }, 0.3);

            // PHASE 1: Deal Andrés's hand (Left side)
            andresCards.forEach((card, index) => {
                const spot  = andresSpots[index] || { x: -300, y: 30, rZ: 0 };
                const inner = card.querySelector('.poker-card-inner');
                const delay = 0.3 + index * 0.15;

                tl.to(card, { x: spot.x * 0.6, y: spot.y - 180, scale: 0.95, opacity: 1, rotationZ: spot.rZ * 0.4, duration: 0.5, ease: "power2.out" }, delay);
                if (inner) tl.to(inner, { rotateY: 180, duration: 0.5, ease: "power1.inOut" }, delay + 0.18);
                tl.to(card, { x: spot.x, y: spot.y, scale: 0.88, rotationZ: spot.rZ, duration: 0.5, ease: "power3.in" }, delay + 0.45);
                tl.to(card, { scaleY: 0.82, scaleX: 0.92, duration: 0.1, ease: "power1.out" }, delay + 0.92);
                tl.to(card, { scaleY: 0.88, scaleX: 0.88, duration: 0.2, ease: "elastic.out(1.2, 0.6)" }, delay + 1.02);
                tl.call(() => { spawnShockwave(card, 90, 'rgba(240, 192, 48, 0.85)'); playTick(400 + index * 30, 0.07); }, null, delay + 0.95);
            });

            // PHASE 2: Deal Johan's hand (Right side)
            const johanStart = 1.4;
            johanCards.forEach((card, index) => {
                const spot  = johanSpots[index] || { x: 300, y: 30, rZ: 0 };
                const inner = card.querySelector('.poker-card-inner');
                const delay = johanStart + index * 0.15;
                ...
                tl.to(card, { scaleY: 0.82, scaleX: 0.92, duration: 0.1, ease: "power1.out" }, delay + 0.92);
                tl.to(card, { scaleY: 0.88, scaleX: 0.88, duration: 0.2, ease: "elastic.out(1.2, 0.6)" }, delay + 1.02);
                tl.call(() => { spawnShockwave(card, 90, 'rgba(17, 212, 131, 0.85)'); playTick(440 + index * 30, 0.07); }, null, delay + 0.95);
            });

            // PHASE 3: THE FINAL SLAM — Central VANTA Master Winning Card
            const masterStart = johanStart + 1.3;
            if (masterCard) {
                const inner = masterCard.querySelector('.poker-card-inner');

                // Fly in from high altitude with massive scale
                tl.to(masterCard, {
                    x: 0, y: 20,
                    scale: 1.15,
                    opacity: 1,
                ...
                // Hard landing impact (Slam + Bounce + Shake + Giant Shockwave)
                tl.to(masterCard, {
                    scale: 1.0,
                    duration: 0.2,
                    ease: "power1.out"
                ...
                tl.call(() => {
                    spawnShockwave(masterCard, 260, '#f0c030');
                    triggerScreenShake();
                    playTick(880, 0.25);
                }, null, masterStart + 0.76);
            }

            // ─── Interactive: Hover lift + Manual flip on click ────────────
            cards.forEach((card) => {
                const inner = card.querySelector('.poker-card-inner');
                const isAndres = card.classList.contains('card-andres');
                const isMaster = card.classList.contains('master-vanta-card');

                card.addEventListener('mouseenter', () => {
                    playTick(560, 0.05);
                    const baseScale = isMaster ? 1.0 : 0.88;
                    gsap.to(card, {
                        y: "-=30", scale: baseScale * 1.15,
                        zIndex: 40, duration: 0.3, ease: "power2.out"
                    });
                });
                card.addEventListener('mouseleave', () => {
                    const baseScale = isMaster ? 1.0 : 0.88;
                    gsap.to(card, {
                        y: "+=30", scale: baseScale,
                        zIndex: 10, duration: 0.3, ease: "power2.out"
                    });
                });
                card.addEventListener('click', (e) => {
                    ...
                    gsap.to(inner, { rotateY: nextRot, duration: 0.65, ease: "back.out(1.5)" });
                });
            });
...
start_marker = '/* ============================================================\n       AWWWARDS CYBERPUNK POKER TECH DECK'
...
if idx_start == -1:
    ...
    if idx_start != -1:
        # back up to start of comment block if any
        idx_comment = content.rfind('/* ============================================================', 0, idx_start)
        if idx_comment != -1:
            idx_start = idx_comment

idx_end = content.find('if (document.readyState === \'loading\') {')

if idx_start != -1 and idx_end != -1:
    new_content = content[:idx_start] + js_poker_code + '\n\n' + content[idx_end:]
    with open('effects.js', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print('SUCCESS: Updated effects.js with full poker dealer logic and 13 cards!')
else:
    print(f'ERROR: Could not find markers in effects.js. idx_start={idx_start}, idx_end={idx_end}')
