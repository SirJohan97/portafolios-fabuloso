...
js_headsup_code = """    /* ============================================================
    ...
    (function initCyberpunkPokerDeckScrollytelling() {
        ...
        function setupPokerDealer() {
            ...
            // ─── DOM Shockwave ────────────────────────────────────────────
            function spawnShockwave(cardEl, customSize = 100, customBorder = 'rgba(17, 212, 131, 0.9)') {
                if (!stage) return;
                const sRect = stage.getBoundingClientRect();
                const cRect = cardEl.getBoundingClientRect();
                const x = (cRect.left + cRect.width / 2) - sRect.left;
                const y = (cRect.bottom) - sRect.top - 8;
            ...
            // ─── Heads-Up Positions (Top = Andrés, Bottom = Johan) ─────────
            ...
            andresCards.forEach((card, i) => {
                ...
                gsap.set(card, { x: 0, y: -180, scale: 0.2, opacity: 0, rotationZ: 0, zIndex: i + 1 });
            ...
            // Reveal Player Seats (Top & Bottom)
            ...
            andresCards.forEach((card, index) => {
                const spot  = andresSpots[index] || { x: 0, y: -150, rZ: 0 };
                const inner = card.querySelector('.poker-card-inner');
                const delay = 0.3 + index * 0.14;

                tl.to(card, { x: spot.x, y: spot.y - 40, scale: 1.0, opacity: 1, rotationZ: spot.rZ * 0.5, duration: 0.45, ease: "power2.out" }, delay);
                if (inner) tl.to(inner, { rotateY: 180, duration: 0.45, ease: "power1.inOut" }, delay + 0.15);
                tl.to(card, { x: spot.x, y: spot.y, scale: 0.92, rotationZ: spot.rZ, duration: 0.45, ease: "power3.in" }, delay + 0.4);
                tl.to(card, { scaleY: 0.86, scaleX: 0.96, duration: 0.1, ease: "power1.out" }, delay + 0.85);
                tl.to(card, { scaleY: 0.92, scaleX: 0.92, duration: 0.18, ease: "elastic.out(1.2, 0.6)" }, delay + 0.95);
                tl.call(() => { spawnShockwave(card, 80, 'rgba(240, 192, 48, 0.85)'); playTick(400 + index * 30, 0.07); }, null, delay + 0.88);
            });

            // PHASE 2: Deal Johan's hand (Bottom of Table)
            const johanStart = 1.35;
            johanCards.forEach((card, index) => {
                const spot  = johanSpots[index] || { x: 0, y: 150, rZ: 0 };
                const inner = card.querySelector('.poker-card-inner');
                const delay = johanStart + index * 0.14;
                ...
                tl.to(card, { scaleY: 0.86, scaleX: 0.96, duration: 0.1, ease: "power1.out" }, delay + 0.85);
                tl.to(card, { scaleY: 0.92, scaleX: 0.92, duration: 0.18, ease: "elastic.out(1.2, 0.6)" }, delay + 0.95);
                tl.call(() => { spawnShockwave(card, 80, 'rgba(17, 212, 131, 0.85)'); playTick(440 + index * 30, 0.07); }, null, delay + 0.88);
            });

            // PHASE 3: THE FINAL SLAM — Central VANTA Master Winning River Card
            const masterStart = johanStart + 1.25;
            if (masterCard) {
                const inner = masterCard.querySelector('.poker-card-inner');

                // Fly in from high altitude with massive scale into center pot
                tl.to(masterCard, {
                    x: 0, y: 0,
                    scale: 1.1,
                    opacity: 1,
                ...
                tl.call(() => {
                    spawnShockwave(masterCard, 220, '#f0c030');
                    triggerScreenShake();
                    playTick(880, 0.22);
                }, null, masterStart + 0.71);
            }
...
if idx_start != -1 and idx_end != -1:
    new_content = content[:idx_start] + js_headsup_code + '\n\n' + content[idx_end:]
    with open('effects.js', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print('SUCCESS: Updated effects.js with Heads-Up Top/Bottom poker sequence!')
else:
    print(f'ERROR: Could not locate markers in effects.js. idx_start={idx_start}, idx_end={idx_end}')
