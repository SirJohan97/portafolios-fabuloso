...
css_clean_styles = """/* =========================================
   REAL HEADS-UP POKER TABLE & COMPACT CARDS (OPTIMIZED & CALIBRATED)
   ========================================= */
.poker-dealer-section {
    position: relative;
    width: 100%;
    min-height: 140vh; /* Calibrated scroll height — no huge empty black space */
    background: #030609;
    overflow: hidden;
}
...
/* HEADER: Always Visible & Balanced at Top */
.poker-header {
    text-align: center;
    max-width: 750px;
...
/* REAL POKER TABLE CONTAINER (ELEGANT HEADS-UP FELT) */
...
.poker-table-felt {
    ...
    height: 100%;
    border-radius: 220px;
    background: radial-gradient(ellipse at center, #0f2e21 0%, #061a12 65%, #020b08 100%);
...
/* PLAYER SEATS — POSITIONED ON TABLE RAILS (ZERO OVERLAP WITH CARDS) */
.player-seat {
    position: absolute;
    left: 50%;
...
if idx_start != -1 and idx_end != -1:
    new_content = content[:idx_start] + css_clean_styles + '\n\n' + content[idx_end:]
    with open('styles.css', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print('SUCCESS: Updated styles.css cleanly!')
else:
    print(f'ERROR: Could not locate markers in styles.css. idx_start={idx_start}, idx_end={idx_end}')
