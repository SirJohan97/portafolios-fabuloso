...
js_table_code = """    /* ============================================================
    ...
    (function initCyberpunkPokerDeckScrollytelling() {
        ...
        function setupPokerDealer() {
            ...
            // ─── Heads-Up Positions (Expanded Table 1080px x 560px) ───────
            ...
            andresCards.forEach((card, index) => {
                const spot  = andresSpots[index] || { x: 0, y: -130, rZ: 0 };
                const inner = card.querySelector('.poker-card-inner');
            ...
            johanCards.forEach((card, index) => {
                const spot  = johanSpots[index] || { x: 0, y: 130, rZ: 0 };
                const inner = card.querySelector('.poker-card-inner');
...
if idx_start != -1 and idx_end != -1:
    new_content = content[:idx_start] + js_table_code + '\n\n}\n\n' + content[idx_end:]
    with open('effects.js', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print('SUCCESS: Updated effects.js with expanded table layout and refined shockwaves!')
else:
